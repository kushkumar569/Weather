...
city.forEach(element => {
    // console.log(element)
    fetch(`http://api.weatherapi.com/v1/current.json?key=29e4d42b777d459cac6112945242506&q=${element}`)
    .then((response) => {
...
fetch(`http://api.weatherapi.com/v1/current.json?key=29e4d42b777d459cac6112945242506&q=sasaram`)
        .then((response) => {
            return response.json()
        })
        .then((data) => {
...
let submit = document.getElementById('btm')

submit.addEventListener('click', function (e) {
    e.preventDefault();

    const search = (document.getElementById('search').value);
    fetch(`http://api.weatherapi.com/v1/current.json?key=29e4d42b777d459cac6112945242506&q=${search}`)
        .then((response) => {
            return response.json()
        })
        .then((data) => {
